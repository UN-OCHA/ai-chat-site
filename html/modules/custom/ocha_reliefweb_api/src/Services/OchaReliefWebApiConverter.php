...
namespace Drupal\ocha_reliefweb_api\Services;
 ...
 */
class OchaReliefWebApiConverter {
  ...
  public function __construct(
    ...
    $this->config = $config_factory->get('ocha_reliefweb_api.settings');
    $this->time = $time;
    $this->httpClient = $http_client;
    $this->logger = $logger_factory->get('ocha_reliefweb_api');
  }
   ...
   */
  public function getApiRequest($url, $timeout = 5, $cache_enabled = TRUE) {
    ...
    if (isset($response)) {
      ...
      if (!empty($data['output']['requests']['post'])) {
        $request = $data['output']['requests']['post'];
      }
    }
    ...
    if ($cache_enabled) {
      ...
      $this->cache->set($cache_id, $request, $cache_expiration, $tags);
    }
